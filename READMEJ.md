...
| GPU | ROCm 7.14.0（`/opt/rocm`、gfx1151 ネイティブビルド。`HSA_OVERRIDE_GFX_VERSION` は**設定しない**） |
...
| llama.cpp（ROCm ビルド） | `~/llama.cpp/build/bin/llama-server` | gfx1151 ネイティブでビルド（`cmake -DGGML_HIP=ON -DAMDGPU_TARGETS=gfx1151`） |
